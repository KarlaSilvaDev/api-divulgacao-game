...
def editar_personagem(id):
    if request.method == "POST":
        ...
        (
            ...
            historia,
            imagem,
        ) = db_utils.retornar_personagem(id)

        return render_template(
            "/admin/editar-personagem.html",
            id=id,
            nome=nome,
            ocupacao=ocupacao,
            forca=forca,
...
def editar_usuario(id):
    if request.method == "POST":
        if "excluir" in request.form:
            ...
            if dados_retornados:
                db_utils.editar_usuario(
                    id=id, nome=nome, login=login, email=email, senha=senha
                )
            else:
                db_utils.adicionar_usuario(
...
def home_habilidade_admin():
    ...
    return render_template(
        "admin/listar-habilidades.html", habilidades=lista_habilidades
    )
...
def editar_habilidade(id):
    if request.method == "POST":
        ...
        return render_template(
            "/admin/editar-habilidade.html", id=id, nome=nome, descricao=descricao
        )
